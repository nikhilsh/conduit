...
#
# ── Agent memwatch (OOM watchdog) ───────────────────────────────────────────
# When user-level systemd is available (same gate as the broker), a small
# watchdog is installed next to it that kills any runaway agent CLI process
# (claude / codex / gemini / opencode) before it can exhaust the box — a
# single agent worker OOM'd a 3.8 GB box on 2026-06-24. Source of truth:
# scripts/agent-memwatch.sh (embedded below verbatim so this one-shot bootstrap
# script stays self-contained). Best-effort: any failure here is logged and
# never aborts bootstrap or affects the OK/ERR line — the broker is what the
# app depends on.
# ────────────────────────────────────────────────────────────────────────────
...
# ── Install agent-memwatch (OOM watchdog; best-effort, never fails bootstrap) ─
# Only attempted when the broker itself landed on user-systemd — memwatch's
# broker-exclusion relies on `systemctl --user show conduit-broker` finding a
# unit, and there is no reboot-durable place to put it on the pidfile path.
# Source of truth for this embedded copy: scripts/agent-memwatch.sh.
echo "STEP install_memwatch" >&2
if [ "$_use_systemd" = "1" ]; then
  MEMWATCH_SCRIPT="$STATE_DIR/agent-memwatch.sh"
  MEMWATCH_UNIT="$SYSTEMD_UNIT_DIR/conduit-agent-memwatch.service"

  cat > "$MEMWATCH_SCRIPT" <<'MEMWATCH_SH'
#!/usr/bin/env bash
# conduit agent memory watchdog — installed by remote-bootstrap.sh.
# Source of truth: scripts/agent-memwatch.sh in the conduit repo (kept
# byte-for-byte in sync; see docs/BROKER-REDEPLOY.md "Agent memwatch").
#
# Kills any runaway agent process (claude / codex / gemini / opencode, incl.
# claude's versioned worker subprocesses) BEFORE it can exhaust the box. The
# broker itself is NEVER a target. Two thresholds: HARD kills immediately;
# SOFT kills only if sustained for SOFT_DURATION (transient spikes are fine).
# All tunables are env-overridable from the systemd unit. Set MEMWATCH_DRYRUN=1
# to log "WOULD KILL" instead of killing.
set -u
...
MEMWATCH_SH
  chmod +x "$MEMWATCH_SCRIPT" 2>/dev/null || true

  # Same self-protection posture as the hand-installed root unit this mirrors
  # (OOMScoreAdjust/MemoryMax/Nice may be refused without CAP_SYS_RESOURCE on
  # an unprivileged --user bus — that only degrades the watchdog's own
  # survivability under memory pressure, it does not block it from starting).
  cat > "$MEMWATCH_UNIT" <<MEMWATCH_UNIT
[Unit]
Description=Conduit agent memory watchdog (kills runaway claude/codex/gemini before OOM)
Documentation=file:$MEMWATCH_SCRIPT
...
Environment=MEMWATCH_LOG=$STATE_DIR/agent-memwatch.log
Environment=MEMWATCH_SYSTEMCTL_SCOPE=--user
ExecStart=$MEMWATCH_SCRIPT
Restart=always
RestartSec=5
OOMScoreAdjust=-900
MemoryMax=128M
Nice=-5
StandardOutput=null
StandardError=append:$STATE_DIR/agent-memwatch.log
...
MEMWATCH_UNIT
  ...
  if systemctl --user enable conduit-agent-memwatch >/dev/null 2>&1 \
     && { systemctl --user restart conduit-agent-memwatch >/dev/null 2>&1 \
          || systemctl --user start conduit-agent-memwatch >/dev/null 2>&1; }; then
    echo "conduit: agent-memwatch started via user-systemd (OOM protection active)" >&2
  else
    echo "conduit: agent-memwatch systemd enable/start failed; continuing without it" >&2
  fi
else
  echo "conduit: user-systemd not available; skipping agent-memwatch (no OOM watchdog on this host)" >&2
fi
