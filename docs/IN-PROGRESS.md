...
- **`/clear` command + LA answered nudge** — broker: Claude /clear pass-through (suppress synthetic "(no content)", publish confirmation), codex /clear broker-orchestrated thread/start, `supports.clear` capability flag, LA pending clears on answer. Branch: `feat/clear-command`, PR: TBD.
