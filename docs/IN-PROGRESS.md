...
- **Session tasks UI PR 1/4** (TaskRow + spinner) -- inline background-task card row (spinner/dot, status text, optional rich tail line) + calm indeterminate task spinner atoms, iOS + Android, previews only (no screen integration yet). Branch `session-tasks-pr1`.
