...
`http://localhost:1455/auth/callback` — never `conduit://...`. By
...
- **Stage 1 — iOS end-to-end (PARTIALLY SHIPPED)**:
  ...
  - **Outbound side NOT wired**: `SessionStore.startAgentLogin(...)` /
    ...
    in `core/src/conduit_core.udl` yet. **Next concrete action**:
...
+ `conduit://` custom-scheme approach. It is preserved verbatim for
historical context (PRs #99, #100, #104, #110, #112). **All sections
below are deprecated. Do not implement against them.**

The original goal: replace the host-side `conduit-mirror-auth`
...
`/usr/local/bin/conduit-mirror-auth` (installed by `scripts/install.sh`
and re-run on every broker start via `ExecStartPre`). Container `home`
volume `conduit-home` then surfaces those files at `/home/app/.claude`
...
- It blocks the litter-style story we want for the website ("install
  the app, log in to ChatGPT / Claude in-app, point at any conduit
  broker, go").
...
we don't need an SSH side-channel). conduit's broker is talking to
...
- **Schema** (from `conduit-mirror-auth` + existing memory):
  ```json
  {
    "claudeAiOauth": {
...
Universal-link / scheme: **`conduit://oauth/<provider>/callback?code=...&state=...`**.
...
```swift
// Service: "conduit.agent-credentials"
...
let attributes: [String: Any] = [
    ...
    kSecAttrService as String: "conduit.agent-credentials",
...
`apps/ios/Sources/Conduit/Views/AgentLoginSheet.swift` (new) — a
...
`ConduitClient.setAgentCredentials(_ provider:, blob:)` sends the
...
Add `conduit` URL scheme + an `Info.plist` `CFBundleURLTypes` entry,
*and* an apple-app-site-association on the conduit website so
`https://conduit.app/oauth-bounce/<provider>` is also a valid
...
<activity android:name=".auth.AgentOAuthRedirectActivity"
    ...
    <intent-filter android:autoVerify="false">
        ...
        <data android:scheme="conduit" android:host="oauth" />
    </intent-filter>
</activity>
```
...
val prefs = EncryptedSharedPreferences.create(
    context, "conduit-agent-credentials", masterKey,
...
`/home/app/.{claude,codex}` files from `conduit-mirror-auth`).
...
- **Keep `conduit-mirror-auth`** as a fallback. The broker's
...
- **Stage 6 — retire `conduit-mirror-auth`.** Once Stage 4 ships and
