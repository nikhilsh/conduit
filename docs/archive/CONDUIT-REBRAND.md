...
- **Phase 7 — visual re-skin: DONE (CI-only verify; on-device check pending).** Implemented the **terminal-daemon `ConduitMark`** as a vector on both platforms (SwiftUI `Canvas` + Compose `Canvas`, exact BRAND.md spec) and rewired every brand-mark surface off the cat raster: iOS `AnimatedBrandMark` + sessions rail + empty detail + tablet bar → vector; splash asset `KittyMark.imageset`→`ConduitMark.imageset` (daemon `LogoMark.png`) + dropped the `pawprint.fill` fallback (→`terminal.fill`); **AppIcon** = daemon 1024. Android `AnimatedBrandMark` + splash → vector; **launcher mipmaps regenerated** as the daemon (all densities, legacy + adaptive foreground) + adaptive bg `#04050A`; dropped `kitty_mark.png`. Palette/fonts already matched BRAND.md (no change). **Zero litter/kitty in apps**; reworded ~152 upstream-provenance comments (`litter`→`upstream`), renamed `PLAN-LITTER-*`→`PLAN-CONDUIT-*` docs, cleaned `kitty-*` test fixtures + 2 user-facing "(litter pattern)" strings. **Polish deferred to device pass:** `>conduit` prompt-styled wordmark (currently plain `"conduit"`); bundling real JetBrains Mono/Space Grotesk font files (system fallback today).
- **Phase 8 — website rebrand: DONE (kept current site).** `build.mjs`: title/meta/tagline → COPY_DECK ("Your agents, in your pocket."), theme-color `#04050a`, repo ref → `nikhilsh/conduit`; swapped cat favicons/hero icons for daemon (`favicon`/`icon-128`/`icon-192` from handoff); `DEPLOY.md` rebranded. `build.mjs` syntax OK. Kept domain `swekitty.kaopeh.com` (decision below).
- **Verification grep:** shipped src is clean of `swe-kitty`/`kitty`/`litter` **except**: the website domain `swekitty.kaopeh.com`/`slug` (deferred), the genuine `dnakov/litter` upstream-credit link in README (real external project — kept), `docs/archive/` history, the `.swe-kitty/` dev dir, and `Conduit_Handoff/` (source of truth).
- _Phase 9 — release + device verify:_ **NEEDS USER** (repo rename, Sentry, broker redeploy + re-pair, provisioning).
