...
## Scope
The production-side agent adapters (lives at repo-root `agents/`, separate from dev-time `.swe-kitty/agents/`). Make `switch_agent` work end-to-end.

**In scope:**
- `agents/claude.toml`, `agents/codex.toml` — production adapter contracts (see `docs/AGENT-ADAPTERS.md`)
- `harness/docker/claude.Dockerfile` — image `swekitty/claude:latest`. Installs `claude` CLI. Entrypoint reads `/workspace/HANDOFF.html` and passes it as system-prompt prefix.
- `harness/docker/codex.Dockerfile` — image `swekitty/codex:latest`. Installs OpenAI `codex` CLI. Entrypoint pattern same as claude.
...
## Done means
- `docker build -t swekitty/claude:latest -f harness/docker/claude.Dockerfile .` succeeds; same for codex
