...
- `harness/internal/session/recovery.go` — on `swe-kitty-harness up`, reattach to existing sessions on disk
- `harness/internal/memory/` — HTML schema validator, render/checkpoint/handoff/promote operations
- `harness/cmd/swe-kitty-harness/memory.go` — `swe-kitty memory {init,render,checkpoint,handoff,promote,show}` subcommands
...
- `swe-kitty memory render --session <uuid>` produces validator-passing HTML
...
- `harness/cmd/swe-kitty-harness/memory.go`
- `.swe-kitty/memory/index.html` (only seed updates)
