...
func TestChatProcessRoundTrip(t *testing.T) {
	...
	cp, err := startChatProcess(ctx, command, nil, "", func(p []byte) { events <- p }, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func TestChatProcessSendAndClose(t *testing.T) {
	...
	cp, err := startChatProcess(context.Background(), command, nil, "", func([]byte) {}, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("start: %v", err)
	}
...
func TestChatProcessExitNotice(t *testing.T) {
	...
	cp, err := startChatProcess(context.Background(), command, nil, "", func(p []byte) { events <- p }, nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("start: %v", err)
	}
	defer cp.Close()
