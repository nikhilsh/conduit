...
func (s *Session) StatusPayload() map[string]any {
	...
	payload := map[string]any{
		...
		// working, so it can't drive a transient indicator — clients fold
		// this instead to clear/keep the composer's working state on
		// (re)connect. False for the legacy TUI-scrape path. Rides every
		// status frame, so the existing turn-end broadcast (accumulateUsage)
		// and the watchdog tick flip it live, not only on reconnect.
		"turn_active": s.TurnActive(),
	}
