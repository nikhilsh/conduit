...
This directory is read by **`conduit-broker`** so multiple AI agents can
work on conduit in parallel, each on its own git worktree, each in its
own PTY-backed session.

> Earlier iterations of this README pointed at upstream `swe-swe` for the
> dev workflow. conduit now ships its own broker binary (`conduit-broker`) and
> is no longer a swe-swe consumer. Don't install upstream swe-swe alongside.
...
- `.conduit/agents/` — what the broker reads when working **on this repo**
  (dev-time). These TOMLs are tuned for the agents that work on conduit
  itself.
- `agents/` (repo root) — what `conduit-broker` ships **to users**. End
  users get this set when they run the broker against their own project.
...
make broker                                     # build ./broker/bin/conduit-broker
./broker/bin/conduit-broker up --local        # opens http://localhost:1977
```
