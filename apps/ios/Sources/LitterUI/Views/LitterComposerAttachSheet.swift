...
// MARK: - LitterComposerAttachBar
//
// The LitterUI composer's "+" attach affordance. Splits into:
...
// back as a `LitterUI.ComposerAttachment`. The composer's send path
...
//
// Matches the LitterUI palette/typography and never appears in
...
extension LitterUI {
    ...
    struct ComposerAttachmentChips: View {
        ...
        private func chip(for attachment: ComposerAttachment) -> some View {
            ...
            .padding(.vertical, 6)
            .frame(maxWidth: 220)
            .litterGlassCapsule(tint: LitterUI.Palette.surfaceLight.color, config: .pill)
        }
    }
    ...
    struct ComposerAttachButton: View {
        ...
        var body: some View {
            Menu {
                Button {
                    ...
                    Label(LitterUI.AttachKind.image.title, systemImage: LitterUI.AttachKind.image.systemImage)
                }
                Button {
                    showFileImporter = true
                } label: {
                    Label(LitterUI.AttachKind.file.title, systemImage: LitterUI.AttachKind.file.systemImage)
                }
            } label: {
        ...
        @MainActor
        private func ingest(kind: LitterUI.AttachKind, filename: String, mime: String, bytes: Data) {
            guard LitterUI.ComposerAttachmentLimits.isWithinLimit(bytes.count) else {
                if bytes.isEmpty {
                    onError("\"\(filename)\" is empty.")
                } else {
                    let mb = LitterUI.ComposerAttachmentLimits.maxBytes / (1024 * 1024)
                    onError("\"\(filename)\" is too large (limit \(mb)MB).")
                }
                return
            }
            onAttach(
                LitterUI.ComposerAttachment(
                    kind: kind,
                    filename: filename,
                    mimeType: mime,
                    bytes: bytes
        ...
        private static func fileMime(for url: URL) -> String {
            ...
            return LitterUI.ComposerMime.fromExtension(url.pathExtension)
        }
    }
}
