...
struct PKCETests {
    ...
    @Test func extractAuthorizationCodeReturnsCode() throws {
        let url = URL(string: "conduit://oauth/openai/callback?code=abc-123&state=xyz")!
        let code = try OAuthClient.extractAuthorizationCode(from: url)
        #expect(code == "abc-123")
    }

    @Test func extractAuthorizationCodeThrowsOnMissingCode() {
        let url = URL(string: "conduit://oauth/openai/callback?state=xyz")!
        do {
    ...
    @Test func extractAuthorizationCodeThrowsOnProviderError() {
        ...
        let url = URL(string: "conduit://oauth/openai/callback?error=access_denied")!
        do {
            _ = try OAuthClient.extractAuthorizationCode(from: url)
    ...
    @Test func anthropicProviderConfigMatchesClaudeCLI() {
        ...
        #expect(cfg.issuer == URL(string: "https://claude.com")!)
        ...
        #expect(cfg.authorizeURL == URL(string: "https://claude.com/cai/oauth/authorize")!)
